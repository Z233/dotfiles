...
local function getCurrentContext()
  local result = vscode.eval(string.format([[
    ...
    if (isGitLensDiff && activeTabInput?.original?.path) {
      // Extract SHA from GitLens URI like: gitlens://abc123def/path/to/file
      const match = activeTabInput.original.path.match(/^\/([a-f0-9]+)\//);
      if (match) {
        revisionSha = match[1];
      }
    }
...
function M.goToPreviousRevision()
  ...
  local stack_size_before = #history.stack
  local index_before = history.current_index
  local tracked_file = history.file_path or "nil"
  local tracked_line = history.line_number or "nil"
